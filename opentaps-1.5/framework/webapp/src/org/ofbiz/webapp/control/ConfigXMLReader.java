...
public class ConfigXMLReader {
  ...
  public static UtilCache<URL, ControllerConfig> controllerCache = UtilCache
      .createUtilCache("webapp.ControllerConfig");
  public static UtilCache<String, List<ControllerConfig>> controllerSearchResultsCache = UtilCache
  ...
  private static ControllerConfig injectControllerIfNeeded(URL url,
      ControllerConfig controllerConfig) {
    // include the controllers to be injected
    try {
      Delegator delegator = DelegatorFactory.getDelegator("default");
      // we need a way to get the injects from the DB, we assume most controllers are after WEB-INF,
      // the corresponding app is one level down eg:
      ...
      if (app == null) {
        ...
        List<GenericValue> injects = delegator.findAllCache("ControllerInjection",
        ...
        for (GenericValue inject : injects) {
          ...
          try {
            ...
            Debug.logError(e,
                "Error reading potential injection target [" + inject.getString("injectIntoUrl")
                    + "]", module);
          }
          ...
          try {
            ...
            if (!controllerConfig.includes.contains(url)) {
              // to avoid an infinite recursion, add #injected to the url included in this way (the
              // URL still resolve to same file)
              // this is because if we inject A into B, B becomes A include(B), which still needs to
              // get B to read the included objects, which will be A(B) again ...
              // instead B becomes A include B#injected, and get B#injected will be just the real B
              // instead of A(B)
              controllerConfig.includes.add(new URL(url.toString() + "#injected"));
            }
          } catch (MalformedURLException e) {
            Debug.logError(e, "Error reading getting controller to inject [" + injectUrl + "]",
                module);
          }
        }
      }
      ...
      for (Element includeElement : UtilXml.childElementList(rootElement, "include")) {
        ...
        if (UtilValidate.isNotEmpty(includeLocation)) {
          try {
            ...
            Debug.logError(mue,
                "Error processing include at [" + includeLocation + "]:" + mue.toString(), module);
          }
        }
      }
    }
  ...
  public static Set<String> findControllerFilesWithRequest(String requestUri,
      String controllerPartialPath) throws GeneralException {
    ...
    List<ControllerConfig> controllerConfigs = (List<ControllerConfig>) controllerSearchResultsCache
        .get(cacheId);

    if (controllerConfigs == null) {
      try {
        // find controller.xml file with webappMountPoint + "/WEB-INF" in the path
        List<File> controllerFiles = FileUtil.findXmlFiles(null, controllerPartialPath,
            "site-conf", "site-conf.xsd");
  ...
  public static Set<String> findControllerRequestUniqueForTargetType(String target, String urlMode)
    ...
    if (indexOfDollarSignCurlyBrace >= 0
      ...
      // we have an expanded string in the requestUri part of the target, not much we can do about
      // that...
      return null;
    }

    if ("intra-app".equals(urlMode)) {
      // look through all controller.xml files and find those with the request-uri referred to by
      // the target
      String requestUri = UtilHttp.getRequestUriFromTarget(target);

      Set<String> controllerLocAndRequestSet = ConfigXMLReader.findControllerFilesWithRequest(
          requestUri, null);
      // if (controllerLocAndRequestSet.size() > 0)
      // Debug.logInfo("============== In findRequestNamesLinkedtoInWidget, controllerLocAndRequestSet: "
      // + controllerLocAndRequestSet, module);
  ...
  public static class RequestMap {
    ...
    public RequestMap(Element requestMapElement) {
      ...
      if (securityElement != null) {
        ...
        this.securityDirectRequest = !"false"
            .equals(securityElement.getAttribute("direct-request"));
      }
  ...
  public static final RequestResponse emptyNoneRequestResponse = RequestResponse
  ...
  public static class RequestResponse {
    ...
    public RequestResponse(Element responseElement) {
      ...
      for (Element redirectParameterElement : UtilXml.childElementList(responseElement,
          "redirect-parameter")) {
      ...
      // NB: the allow-params and allow-attributes list are now implemented both the same as
      // redirect-parameter from above
      ...
      // the allowed params list does the same as the redirect-parameter list above but is less
      // flexible
      ...
      // although previously the allowed-attributes would only be transferred from the response
      // attributes
