...
public final class ProductImportServices {
    ...
    private static List<GenericValue> decodeProduct(GenericValue data, Timestamp now, String goodIdentificationTypeId1, String goodIdentificationTypeId2, Delegator delegator) throws GenericEntityException, Exception {
        ...
        if(data.get("category")!=null)
        {
            ...
            input.put("productCategoryId", data.get("category"));
            input.put("fromDate", now);  
            Debug.logInfo("Categoria " + data.get("category"), MODULE);
        }
        
        GenericValue category = delegator.makeValue("ProductCategoryMember", input);
        toStore.add(category);
        
        
        ////// Relation glAccount and Product
        if(data.get("glAccountId")!=null)
        {
        	
            String glAccountProduct = data.getString("glAccountId");
            Debug.logInfo("glAccount " + data.getString("glAccountId"), MODULE);
            glAccountProduct = glAccountProduct.trim();
           
            GenericValue productglaccount = delegator.findOne("GlAccount", UtilMisc.toMap("glAccountId", glAccountProduct), false);
            Debug.logInfo("tYPE " +productglaccount.get("glAccountTypeId"), MODULE);
            input = FastMap.newInstance();           
            input.put("productId", product.get("productId"));            
            input.put("glAccountId", data.get("glAccountId"));
            input.put("glAccountTypeId", productglaccount.get("glAccountTypeId"));
            input.put("organizationPartyId", data.get("organization"));           
                    
        }
        
        GenericValue accountproduct = delegator.makeValue("ProductGlAccount", input);
        toStore.add(accountproduct);
