...
public class TagImportService extends DomainService implements
	...
	public void importTag() throws ServiceException{        
        try {
            ...
            TagDataImportRepositoryInterface imp_repo = this.getDomainsDirectory().getDataImportDomain().getTagDataImportRepository();
            LedgerRepositoryInterface ledger_repo  =this.getDomainsDirectory().getLedgerDomain().getLedgerRepository();
            
            List<DataImportTag> dataforimp = imp_repo.findNotProcessesDataImportTags();
            ...
            for(int i = 0; i < dataforimp.size(); i++){
                ...
                try{
                    	...
                    	//Buscar Parent Id del tag
                    	 List<Enumeration> enums1 = ledger_repo.findList(
                        		 Enumeration.class, ledger_repo.map(
                        				 Enumeration.Fields.sequenceId,rawdata.getSequenceNum(),
                        				 Enumeration.Fields.enumTypeId,types.get(0).getEnumTypeId()));
                    ...
                    String message = "Successfully imported Tag [" + rawdata.getId() + "].";                   
                    ...
                    String message = "Failed to import Tag [" + rawdata.getId() + "], Error message : " + ex.getMessage();
...
private void getParentEnumeration(LedgerRepositoryInterface ledger_repo, List<DataImportTag> dataforimp) throws RepositoryException {
		...
		try {
			 ...
			 for(int i = 0; i < dataforimp.size(); i++){
				...
				if(rowdata.getParentId()!= null && rowdata.getSequenceNum()!=null)
				{
					List<Enumeration> enumId = getSequenceEnumeration(ledger_repo,  rowdata);
	 ...
	 */
	private List<Enumeration> getSequenceEnumeration(LedgerRepositoryInterface ledger_repo, DataImportTag rowdata)
			throws RepositoryException {
		
		logger.debug("entro a la funcion  getSequenceEnumeration " + rowdata.getSequenceNum() + ", " + rowdata.getType());
		//Buscar si existe el registro para hacer update	
		 List<Enumeration> sequence = ledger_repo.findList(
		   		 Enumeration.class, ledger_repo.map(
		   				 Enumeration.Fields.sequenceId,rowdata.getSequenceNum(),
		   				 Enumeration.Fields.enumTypeId,rowdata.getType()));
