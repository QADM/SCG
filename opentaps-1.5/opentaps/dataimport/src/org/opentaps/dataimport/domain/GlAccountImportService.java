...
public class GlAccountImportService extends DomainService implements
	...
	public void importGlAccounts() throws ServiceException {
		try {
			...
			List<DataImportGlAccount> dataforimp = imp_repo
					.findNotProcessesDataImportGlAccountEntries();
			...
			// si los saldos son iguales crea la transaccion y guarda el saldo
			// inicial.
	...
	/**
	 * Relacionar Cuentas con su categoria GlAccountCategoryRelation No se puede
	 * repetir el tipo
	 * **/
	private void getRelationCategory(DataImportGlAccount rawdata,
		...
		try {
			if (rawdata.getCatalog() != null) {
				...
				glaccountRelation.setProductCategoryId(rawdata.getCatalog()
						.trim());
				...
				String message = "Successfully relation GlAccountCategoryRelation ["
						+ rawdata.getGlAccountId() + "].";
				Debug.logInfo(message, MODULE);
			}
		} catch (Exception e) {
			String message = "Failed to relation GlAccountCategoryRelation ["
					+ rawdata.getGlAccountId() + "], Error message : "
					+ e.getMessage();
	...
	/**
	 * Definir el tipo de la cuenta GlAccountTypeDefault No se puede repetir el
	 * tipo
	 * **/

	private void getType(DataImportGlAccount rawdata,
			LedgerRepositoryInterface ledger_repo) throws ServiceException {
		Transaction imp_tx1 = null;
		try {
	...
	private void GenerarTransaccion(List<String> Listacuentas,
		...
		try {
			...
			if (!periodo.isEmpty() && periodo.size() > 1) {
				...
				if (actual && anterior) {
					logger.debug("Año actual " + annioactual + "Año anteriror "
							+ annioanter);

					Calendar date = Calendar.getInstance();
					// date.set(Integer.parseInt(annioanter), 11, 31);
					date.set(Integer.parseInt(annioanter), 11, 31, 23, 59, 59);
					...
					acctgsaldo.setCreatedByUserLogin("admin");// Buscar de donde
																// tomarlo
					...
					for (String cuenta : Listacuentas) {
						...
						List<GlAccountOrganization> Listorganizacion = ledger_repo
								.findList(
						...
						if (!Listorganizacion.isEmpty()) {
							for (GlAccountOrganization glorgan : Listorganizacion) {
								...
								//
								// saldofinal =
								// glorgan.getPostedBalance().subtract(saldo);
								// }
								//
								// }
